---
name: 费用展示与标题栏按钮
overview: 在无边框自定义标题栏的关闭按钮左侧加入置顶与历史按钮；每次翻译按 DeepSeek usage 估算花费并展示 cache 节省；按日 JSONL 永久保留，历史窗可按日期回看。
todos:
  - id: pricing-usage
    content: pricing.py + TranslateResult 带 usage 数值并估算费用/节省
    status: in_progress
  - id: custom-titlebar
    content: window.py 无边框标题栏：置顶/历史/最小化/关闭
    status: pending
  - id: history-store-ui
    content: 按日 JSONL 永久保留 + HistoryDialog 选日期/回填
    status: pending
  - id: wire-restart
    content: main.py 串接状态栏账单与历史，重启验证
    status: pending
isProject: false
---

...
托盘 / 菜单栏图标：显示窗口 / 手动输入翻译 / 设置 / 历史记录 / 清空翻译上下文 / 清空问答上下文 / 暂停监听 / 安装浏览器扩展 / 检查更新 / 退出。

## 原文朗读

翻译模式下，「原文」右侧的小喇叭会朗读当前原文；再次点击可停止。含汉字时使用系统中文语音，纯英文使用英语语音；中英混排走中文音色。朗读使用系统离线语音，不上传原文，也不需要额外 API Key。Windows 与 macOS 均支持；缺少对应语言音色时，请在系统语音设置中下载（macOS：「系统设置 → 辅助功能 → 朗读内容 → 系统语音」）。
